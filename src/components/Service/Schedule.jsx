export default function Schedule() {
  return (
    <div className="col-service | tw-flex tw-flex-col tw-justify-center tw-drop-shadow-xl">
      <div className="tw-flex tw-justify-between">
        <span className="services-subtitle">Choose when you want to eat</span>
        <span className="services-line"></span>
      </div>
      <h1 className="services-title tw-mt-8">
        Easy schedule your meal with 1 click
      </h1>
      <span className="services-motto tw-mt-3">
        Don't have to rushing-order anymore when you can plan it
      </span>
    </div>
  );
}
